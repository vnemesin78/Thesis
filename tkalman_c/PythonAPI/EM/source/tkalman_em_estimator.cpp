...
BOOST_PYTHON_MODULE(em)
{
    class_<EM :: estimator>("estimator", init<std::string>())
		...
		if ( j > 0 && likelihood )
		{
			if (  (likelihood[j] - likelihood[j - 1]) / (likelihood[j] + likelihood[j - 1])  > 1e-3 )
			{
				...
				if ( j == 1 )
				{
					...
					cout << endl;
					cout << "Next time, I will kill you!" << endl;
					cout << "The Darth Pairwise Kalman EM algorithm!" << endl;
				}	
